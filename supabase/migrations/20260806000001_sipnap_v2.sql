-- 20260806000001: SIPNAP v2.
-- Adds destruction records, the SARANA sale type, export snapshot columns,
-- a private storage bucket, and the v2 report RPC with source splits and
-- three hard-block checks (continuity, negative, BAP).
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public' AND tablename = 'stock_destructions'
          AND policyname = 'Tenant isolation for stock_destructions'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public' AND tablename = 'stock_destruction_items'
          AND policyname = 'Tenant isolation for stock_destruction_items'
    ) THEN
...
-- ============================================================
-- 2. SARANA sale type (B2B outbound transfer to a facility).
-- ============================================================
DO $$
BEGIN
    IF NOT EXISTS (
...
-- ============================================================
-- 4. Storage bucket for the exact submitted files.
-- Private bucket: download goes through a signed URL.
-- ============================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'storage' AND tablename = 'objects'
          AND policyname = 'Tenant read own sipnap-archives'
    ) THEN
        CREATE POLICY "Tenant read own sipnap-archives" ON storage.objects
            FOR SELECT
            USING (
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'storage' AND tablename = 'objects'
          AND policyname = 'Tenant insert own sipnap-archives'
    ) THEN
        CREATE POLICY "Tenant insert own sipnap-archives" ON storage.objects
            FOR INSERT
            WITH CHECK (
                bucket_id = 'sipnap-archives'
                AND (storage.foldername(name))[1] = (current_setting('request.jwt.claims', true)::jsonb -> 'app_metadata' ->> 'tenant_id')
...
-- ============================================================
-- 5. RPC v2: per-source movements + continuity hard-blocks.
-- SECURITY INVOKER: caller RLS policies scope every table read.
-- ============================================================
...
BEGIN
    ...
    RETURN (
        ...
        -- SARANA rows need only a facility name; RESEP rows need the full set.
        tx_valid AS (
            SELECT *,
                CASE
        ...
        -- Outgoing by type: prescription vs facility transfer.
        sales_out AS (
            SELECT pr.id AS product_id, pr.name AS product_name,
