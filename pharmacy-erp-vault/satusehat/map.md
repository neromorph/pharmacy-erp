...
- [Design: FHIR MedicationDispense payload mapping](issues/07-design-fhir-mapping.md) — ✅ Resolved (grilled + approved): per drug line; `Medication` in `contained` (`#ref` required); doctor IHS lookup mirrors patient pattern; quantity uses `v3-orderableDrugForm` (not UCUM); BPJS payload identical; omit dosageInstruction; racikan = one pair per compound batch with `ingredient[]` + medicationType SD/EP. **LIVE-VERIFIED corrections:** full chain required — Location + Encounter + MedicationRequest + MedicationDispense per drug line (authorizingPrescription MANDATORY, Rule 10393); MedicationDispense identifier system is `http://sys-ids.kemkes.go.id/prescription/{org-id}` (UUID works); Encounter is full-shape (statusHistory, participant=Practitioner, location, diagnosis=real Condition ref); Condition POST blocked for pharmacy class. All 4 POSTs validated 201.

## Not yet specified

- Bundle vs sequential POST for Medication+MedicationDispense — resolved in ticket 07: per drug line (sequential POST, no Bundle)
- Whether `authorizingPrescription` can be omitted for Apotek-only RESEP (no upstream MedicationRequest) — **resolved with correction**: it is MANDATORY; full chain (Location → Encounter → MedicationRequest → MedicationDispense) required per drug line
