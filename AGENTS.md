...
Operational state for the pharmacy-erp project: progress, next steps, infrastructure, and knowledge. Knowledge lives in two Obsidian vaults: the project vault `pharmacy-erp-vault/` in this repo (domain vocabulary, maps, infra details), and the personal second brain at `/Users/mufid/pintar/pintar/` (ADRs, Lessons Learned, cross-project knowledge). Domain vocabulary lives in `pharmacy-erp-vault/CONTEXT.md`; deep infra details in `pharmacy-erp-vault/supabase-deployment.md`.
...
- Record architecture decisions as `ADR-NNN <Title>.md` in the second brain `ADR/` folder, and incident RCAs in `Lessons Learned/`. Link related notes with `[[wikilinks]]`.
...
Single-context: `pharmacy-erp-vault/CONTEXT.md` (in this repo) for domain vocabulary. ADRs and Lessons Learned live in the Obsidian second brain at `/Users/mufid/pintar/pintar/` (`ADR/`, `Lessons Learned/`). See `pharmacy-erp-vault/agents/domain.md`.
